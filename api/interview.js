        ...
        if (fbData) {
            ...
            Object.values(fbData).forEach(item => {
                if (item.firm && item.firm !== "Unspecified Firm") {
                    ...
                    let isFeedback = msgLow.includes("interview") || msgLow.includes("feedback") || msgLow.includes("asked");
                    if (isFeedback) {
                        for (let f of FIRM_MAPPINGS) {
                            if (f.aliases.some(alias => new RegExp(`\\b${alias}\\b`, 'i').test(msgLow))) {
                                interviewFirms.add(f.id); break;
                            }
                        }
                    }
                }
